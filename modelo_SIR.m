...
r=0.0001
k=0.1
f=@(V) [-r*V(1)*V(2),r*V(1)*V(2)-k*V(2),k*V(2)]; %Defino el campo vectorial del problema depredador presa,
...
h=0.01; %quiero dar saltos de 0.1, esto lo pueden modificar ustedes
...
for i=1:length(t) %Recorro todas las entradas del vector que cree ahorita
    zf = RungeKutta_Vec(f,[a,t(i)],[x0,y0,z0],h); %Calculo cuanto es X y Y evaluados en el valor de la iteracón t(i)
